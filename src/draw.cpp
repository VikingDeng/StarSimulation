...
constexpr double BASE_LUMINOSITY = 255.0;
constexpr double MIN_RADIUS = 0.5;
constexpr double MAX_RADIUS = 8.0;
constexpr double PSF_BETA = 2.0;        // 调整 PSF 的 beta 参数
constexpr double PSF_FWHM_SCALE = 2.5; // 调整基础 FWHM 的缩放
constexpr double PSF_KERNEL_SIZE_MULTIPLIER = 4.0; // PSF 核大小的倍数
constexpr double SATURATION_MAGNITUDE = 1.0; // 饱和星等的阈值，比这个亮度高的星会发生饱和
...
void drawStarMap(const std::vector<star>& stars,
    ...
    for (const auto& star : stars) {
        ...
        double radius = std::clamp(intensity * MAX_RADIUS, MIN_RADIUS, MAX_RADIUS);
        ...
        const double originalFWHM = PSF_FWHM_SCALE * radius;
